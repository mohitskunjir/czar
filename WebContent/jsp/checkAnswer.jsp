...
<%
	...
	try
	{
		...
		player.setTime(time);
			...
			if(buttonClicked.equals("Next"))
			{
				...
				ans = question.generateAnswer(curentQuestionNo);
				...
				else
				{
					...
					<script>
						setCookie("quiz","1");
						window.location.replace("../quiz.jsp?msg=check answer null answer");
					</script>
				<%
				}
			}
			...
			{	
				...
				else
				{
					%>
						<script>
							setCookie("quiz","1");
							window.location.replace("../quiz.jsp?msg=No more skips");
						</script>
					<%
				}
			}
	...
	catch (SQLException e)
	{
		if(e.getErrorCode()==1049) // occurs when database is not found
		{
			%>
			<script>				
			   window.location.replace("../index.jsp?msg=Check Answer: Database Not found");
			</script>
			<%
		}
		...
		else if(e.getErrorCode()==1045) // occurs when database credentials are illegal.... check properties
		{
			%>
			<script>				
			   window.location.replace("../index.jsp?msg=Check Answer: Access Denied");
			</script>
			<%
		}
		...
		{	
			%>
			<script>				
			   window.location.replace("../quiz.jsp?msg=Check Answer: Data too long"); 
			</script>
			<%	
		}
		...
		else if( e.getMessage().equals("1") ) // programmer defined occurs when question does not exist
		{
			%>
			<script>				
			   window.location.replace("../quiz.jsp?msg=Check Answer: Question Does not exist"); 
			</script>
			<%	
		}
		...
		else if(e.getErrorCode() == 1062 ) // occurs when specified username already exists in the database  (Won't Occur)
		{
			%>
			<script>				
			   window.location.replace("../index.jsp?msg=Check Answer: User already exist");
			</script>
			<%
		}
		else
		{
			%>
			<script>				
				window.location.replace("../index.jsp?msg=Check Answer: Unknown Database Error"); 
			</script>
			<%
		}
	}
	...
	catch(NullPointerException e)
	{
		%>
		<script>				
		   window.location.replace("../index.jsp?msg=Check Answer: Unknown Null Error");
		</script>
		<%
	}
	catch(ArrayIndexOutOfBoundsException e)
	{
		%>
		<script>				
		   window.location.replace("../quiz.jsp?msg=Check Answer: Question Does not exist Array"); 
		</script>
		<%	
	}
	catch(Exception e)
	{
		%>
		<script>				
		   window.location.replace("../index.jsp?msg=Check Answer: Unknown Error");
		</script>
		<%
	}
%>
