...
try
{
	ques = question.generateQuestion(questionNo);

	if(ques != null)
	{
		options = question.generateOptions(questionNo);
	}
	else
	{
...
catch (SQLException e)
{
	...
	else if(e.getErrorCode() == 1062 ) // occurs when specified username already exists in the database  (Won't Occur)
	{
		%>
			<script>				
			   window.location.replace("quiz.jsp?msg=Check quiz: User already exist");
			</script>
		<%
	}
	else
	{
		%>
			<script>				
				window.location.replace("quiz.jsp?msg=Check quiz: Unknown Database Error"); 
			</script>
		<%
	}
}
...
catch(Exception e)
{
	%>
		<script>				
		   window.location.replace("quiz.jsp?msg=Check quiz: Unknown Error");
		</script>
	<%
}
...
<li class="logo" style="color:darkgray ;font-size:1.5em">Pressing the back button or using back shortcut is strictly prohibited, else you will be blocked</li>
    ...
    <form class="form-horizontal" name="quizForm"  method="post" id="quizForm" action="jsp/checkAnswer.jsp">
	...
	<footer class="raw-footer container-fluid" style="padding : 15px 15px ; background:whitesmoke">
			...
			<div class="row">
				<div class="col-xs-12 col-sm-4">
					...
					<button id="skip" type="button" class="btn btn-default btn-md skipButton" style="background:lightgray" data-toggle="modal" data-target="#skipModal">Skip <%=player.getSkipsRemaining()%></button>
					<script>
